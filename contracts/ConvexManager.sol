...
contract ConvexManager is Ownable {
	...
	constructor (
		...
		_approveToken(_initialCvxAddress, _initialCvxRewardsPoolAddress, maxInt);
	...
	function buyCvx(uint256 slippage) public payable onlyOwner {
		...
		uint256 minAmountOut = _calculateSwapMinAmountOut(msg.value, slippage, _ethToCvxPath);

		require(minAmountOut > 0, "zero min amount.");

		router.swapExactETHForTokens{value: msg.value}(minAmountOut, _ethToCvxPath, address(this), block.timestamp + 30 minutes);
	}
	...
	function swapRewards(uint256 slippage) public onlyOwner {
		...
		uint256 minAmountOut = _calculateSwapMinAmountOut(amountIn, slippage, _cvxCrvToCvxPath);

		require(minAmountOut > 0, "zero min amount.");

		router.swapExactTokensForTokens(amountIn, minAmountOut, _cvxCrvToCvxPath, address(this), block.timestamp + 30 minutes);
	}
	...
	function liquidateAll(uint256 slippage) external onlyOwner {
		...
		uint256 minAmountOut = _calculateSwapMinAmountOut(amountIn, slippage, _cvxToEthPath);

		require(minAmountOut > 0, "zero min amount out.");

		router.swapExactTokensForETH(amountIn, minAmountOut, _cvxToEthPath, msg.sender, block.timestamp + 30 minutes);
	}
	...
	function approveToken(address token, address account, uint256 amount) public onlyOwner {
	...
	function setCvxRewardPool(address _newCvxRewardPoolAddress) external onlyOwner {
	...
	function _calculateSwapMinAmountOut(uint256 _amountIn, uint256 _slippage, address[] memory _path) internal view returns (uint256) {
		uint256[] memory amountsOut = router.getAmountsOut(_amountIn, _path);

		uint256 amountOut = amountsOut[amountsOut.length - 1];

		return (amountOut * (SLIPPAGE_DENOMINATOR - _slippage)) / SLIPPAGE_DENOMINATOR;
	}

	function _approveToken(address _token, address _account, uint256 _amount) internal {
